...
import Style from "./CalendarInfo.module.css"
...
export default function CalendarInfo(props) {
    ...
    function loadDays() {
        ...
        if(props.dateset.length > 0){
            props.dateset.forEach((el,i)=>{
                let dateE = new Date(el.eventDate)
                appyDayStyle2(dateE)//el.date)
                console.log(dateE)//el.date)
            })
    ...
    function getDat2(e, i) {
        if (!props.readOnly) {
            ...
            let val = props.dateset.filter((el,i)=>{
                let dt1 = new Date(el.eventDate)
                let fres = compareDates(dt1, e.full)
                if(fres){
                    return el
                }
            })
   ...
   function appyDayStyle() {
        Array.from(refDays.current.children).forEach((el, i) => {
            ...
            if (lst.length > 0) {
                ...
                let comp = compareDates(selectedDT.current, objDate)
                if (comp) {
                    el.classList.add("selectedDateDayMain")
                } else {
                    el.classList.remove("selectedDateDayMain")
                }
            } else {
                el.classList.remove("selectedDateDayMain")
            }
    ...
    function appyDayStyle2(dateArray) {
        Array.from(refDays.current.children).forEach((el, i) => {
            //console.log(el)
            let lst = el.querySelectorAll('span')
            if (lst.length > 0) {
                let obj = JSON.parse(lst[0].textContent)
                let objDate = new Date(obj)
                let comp = compareDates(dateArray, objDate)
            ...
            <div className={Style.calCon}>
                <div className={Style.mainSide}>
                    ...
                    <div className={Style.CalendarH}>
                        <div className={Style.MMCon}>
                            {monthName.map((el, i) => {
                                return (
                                    <div className={Style.MMonth} onClick={() => gotoMonth(el)} key={`monthLstK${i}`}>{el}</div>
                                )
                            })}
                        </div>
